...
package:
	cd .. && rm -f *.zip && zip -r shapefile_splitter.experimental.zip shapefile_splitter -x \*.pyc \*.ts \*.ui \*.qrc \*.pro \*~ \*.git\* \*.svn\* \*Makefile*
	mv ../shapefile_splitter.experimental.zip .

upload:
	plugin_uploader_NG.py shapefile_splitter.experimental.zip
